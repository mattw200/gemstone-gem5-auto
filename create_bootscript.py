...
def create_rcs_from_preset(workloads_df_path, xu3_data_path, cpu_mask, preset, output_file, include_warmup=True):
    import pandas
    import os
    import sys
    workloads_df = pandas.read_csv(workloads_df_path, sep='\t')
    names_to_include = presets[preset]
    create_rcs(workloads_df, names_to_include, cpu_mask, preset, output_file,include_warmup=True)
    accum = 0
...
def create_rcs(workloads_df, names_to_include, cpu_mask, preset, output_file,include_warmup=True):
    import pandas 
    output_text = "#!/bin/sh\n\n"
    if preset:
